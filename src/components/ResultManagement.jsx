...
function ResultManagement() {
  ...
  try {
    ...
    console.log('RESULT MANAGEMENT DATA:', data);
    ...
    const loadedParticipants = Array.isArray(data.participants)
      ? data.participants
    ...
    const loadedCompetitions = Array.isArray(data.competitions)
      ? data.competitions
      : [];
    ...
    // =========================================
    // 4. LOAD REGISTRATIONS SEPARATELY
    // =========================================
    ...
    try {
      ...
      if (registrationResponse.ok) {
        ...
        if (Array.isArray(registrationData)) {
          loadedRegistrations = registrationData;
        } else if (
  ...
  const renderParticipantOptions = () => {
    ...
    return filteredParticipants.map((student) => {
      const id = getParticipantId(student);
      const name = getParticipantName(student);

      const chest =
        student.chest || student.chestNo;
  ...
  const publishResult = async () => {
    ...
    if (!first) {
      setMessage('⚠️ Please select 1st place.');
      return;
    }

    if (!second) {
      setMessage('⚠️ Please select 2nd place.');
      return;
    }

    if (!third) {
      setMessage('⚠️ Please select 3rd place.');
      return;
    }
    ...
    try {
      ...
      const selectedFirst = participants.find(
        (student) =>
          String(getId(student)) === String(first)
      );

      const selectedSecond = participants.find(
        (student) =>
          String(getId(student)) === String(second)
      );

      const selectedThird = participants.find(
        (student) =>
          String(getId(student)) === String(third)
      );
      ...
      const result = {
        ...
        first: {
          participantId: getId(selectedFirst),
          name: getParticipantName(selectedFirst),
          chest:
            selectedFirst?.chest ||
            selectedFirst?.chestNo ||
            '',
          team: getTeamName(selectedFirst),
        },

        second: {
          participantId: getId(selectedSecond),
          name: getParticipantName(selectedSecond),
          chest:
            selectedSecond?.chest ||
            selectedSecond?.chestNo ||
            '',
          team: getTeamName(selectedSecond),
        },

        third: {
          participantId: getId(selectedThird),
          name: getParticipantName(selectedThird),
          chest:
            selectedThird?.chest ||
            selectedThird?.chestNo ||
            '',
          team: getTeamName(selectedThird),
        },
